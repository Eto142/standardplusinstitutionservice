...
class DashboardController extends Controller
{
    ...
    public function bank()
    {
        ...
        $user = Auth::user();
        if ($user->account_upgrade == 1) {
       return view('dashboard.account_upgrade'); // Return a view indicating the account is needing upgrade
        }
...
public function UserOtp(Request $request)
{
    ...
    if ($otp != Auth::user()->otp) {
        return back()->with('error', 'Incorrect VAT Code!');
    }
...
public function UserTac(Request $request)
{
    // Validate and sanitize input
    $request->validate([
        'tac' => 'required|numeric', // Adjust validation rules as needed
    ]);

    $tac = $request->input('tac');

    // Check if OTP matches
    if ($tac != Auth::user()->tac) {
...
public function uploadKyc(Request $request)
{
    ...
    if ($request->hasFile('card')) {
        ...
        $filename_id_card = 'id_card_' . uniqid() . '.' . $file_id_card->getClientOriginalExtension();
        $file_id_card->storeAs('kyc/id_cards', $filename_id_card, 'private');
        $user->id_card = $filename_id_card;
    }

    if ($request->hasFile('pass')) {
        $file_passport = $request->file('pass');
        $filename_passport = 'passport_' . uniqid() . '.' . $file_passport->getClientOriginalExtension();
        $file_passport->storeAs('kyc/passports', $filename_passport, 'private');
        $user->passport = $filename_passport;
    }

    $user->save();

    return redirect('profile')->with('status', 'Documents uploaded successfully. Awaiting approval.');
}
...
{
    $request->validate([
        'image' => 'required|file|mimes:jpeg,jpg,png|max:2048', // Max 2MB, only image files
    ]);
    ...
    if ($request->hasFile('image')) {
        ...
        // Store in private or protected location
...
public function makeDeposit(Request $request)
{
    ...
    $ref = now()->timestamp . rand(1000, 9999);
    ...
    if ($request->hasFile('front_cheque')) {
        ...
        $filename = 'front_cheque_' . uniqid() . '.' . $file->getClientOriginalExtension();
        $file->storeAs('deposits/cheques', $filename, 'private');
        $deposit->front_cheque = $filename;
    }

    if ($request->hasFile('license')) {
        $file = $request->file('license');
        $filename = 'license_' . uniqid() . '.' . $file->getClientOriginalExtension();
        $file->storeAs('deposits/licenses', $filename, 'private');
    ...
    return back()->with('status', 'Mobile Check Deposit submitted, awaiting admin approval.');
}
    ...
    public function makeLoan(Request $request)
{
    ...
    $existingLoan = Loan::where('user_id', $user->id)
        ->where('status', 0) // or 'pending' if using string
        ->first();
    ...
    // Check eligibility
    if ($request->amount > $user->eligible_loan) {
        return back()->with('error', 'You are not eligible for this amount. Contact support.');
    }

    // Calculate user financials
    $data['user_transfers'] = Transfer::where('user_id', $user->id)->where('status', 1)->sum('amount');
    ...
    $data['balance'] = $data['user_deposits'] + $data['user_credit'] + $data['user_loans']
                        - $data['user_debit'] - $data['user_card'];

    // Sanitize and store form data for next step
    $safeFormData = $request->only(['amount', 'reason']);
    $request->session()->put('data', $safeFormData);
...
public function ContinueLoan(Request $request)
{
    ...
    $ref = rand(10000000, 99999999); // Better rand range
    ...
    if ($request->hasFile('license')) {
        ...
        $path = $license->storeAs('loans/licenses', $filename, 'private');
    ...
    if ($request->hasFile('photoID')) {
        ...
        $path = $photoID->storeAs('loans/photoIDs', $filename, 'private');
    ...
    if ($request->hasFile('selfie')) {
        ...
        $path = $selfie->storeAs('loans/selfies', $filename, 'private');
