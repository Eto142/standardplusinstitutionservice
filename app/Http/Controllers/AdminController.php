...
public function AccountUpgrade($id)
{
    
 
    $user = array();
    $user['account_upgrade'] = 1;
    $update = DB::table('users')->where('id',$id)->update($user);
   
    return back()->with('status', 'Account Upgrade Activated');  
    
}

public function iAccountUpgrade($id)
{
    
 
    $user = array();
    $user['account_upgrade'] = 0;
    $update = DB::table('users')->where('id',$id)->update($user);
   
    return back()->with('status', 'Account Upgrade Deactivated');  
...
public function updateOtp(Request $request, $id)
{
    ...
    return back()->with('status', 'user VAT Code Updated Successfully');  
    
}



public function updateTac(Request $request, $id)
{
